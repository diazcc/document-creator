...
<template>
    <select 
        ...
        :class="['select select--' + dataSelect?.className  + ' ' + (dataSelect?.options.length<1 ? 'disabled' : '')]"
    ...
    >
        <option :class="'select__' + (dataSelect?.multiple ? 'multiple' : '')" :selected="dataSelect?.model == ''" disabled value=''>
            {{ t(dataSelect.placeholder) }} 
        </option> 
        <option v-if="dataSelect?.options.length >= 1" v-for="option in dataSelect?.options" :selected="option?.value === dataSelect?.model" :key="option?.value" :value="option?.value" class="select__option">
